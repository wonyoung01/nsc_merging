...
from models import KernelMergeModel, LinearCombLoss, SimpleMergeModel
...
def get_rep_metric(cfg):
    if cfg.task.name in ["classification", "sequence_classification"]:
        return "top1_acc"
    elif cfg.task.name == "vqa" and cfg.data.name.lower() == "vizwiz_vqa":
        return "exact_match"
    elif cfg.task.name == "vqa" and cfg.data.name.lower() == "iconqa":
        return "accuracy"
    elif cfg.task.name == "vqa" and cfg.data.name.lower() == "chartqa":
        return "relaxed_accuracy"
    elif cfg.task.name == "vqa" and cfg.data.name.lower() == "docvqa":
        return "ANLS"
    elif cfg.task.name == "captioning":
        return "CIDEr"
    else:
        raise ValueError(
...
def get_validation_fn(cfg):
    if cfg.task.name in ["classification", "sequence_classification"]:
        ...
        utils = import_module("vlm.utils")
        return utils.evaluate_llava
    else:
...
def get_adapter_save_path(cfg, model_dir):
    # If model_dir is indeed a directory, append the adapter save name
...
def get_decoder_save_path(cfg, model_dir):
    # If model_dir is indeed a directory, append the decoder save name
...
def dataloader_helper(cfg, batch_size=None, is_linsearch=False):
    """
    Helper function to get dataloaders for validation and testing.
    """
    if is_linsearch:
        cfg.dataloader.val_batch_size = 16
        cfg.dataloader.val_num_workers = 4
    cfg.dataloader.test_batch_size = 16
...
def train_merged_model(
    ...
    if track or kernel:
        merged_model = KernelMergeModel(**kwargs)
    else:
        merged_model = SimpleMergeModel(**kwargs)
    merged_model.to(device)
    # NOTE: It may cause OOM if projector cluster is large
    if hasattr(merged_model, "projector_cluster"):
        merged_model.projector_cluster.send_to_device(device)
    ...
    # If instance of VQA or Captioning, we need to handle the inputs differently
    if task in ["vqa", "captioning"]:
        from src.vlm.utils import get_collate_fn

        processor = merged_model.models[0].processor  # type: ignore
        for cfg, val_loader in zip(cfgs, val_loaders):
            val_loader.collate_fn = get_collate_fn(
                cfg,
                processor,
                with_answer=False,
                with_meta=False,
    ...
    # Cycle through the validation loaders to create infinite iterators
    cycled_val_loaders = [cycle(val_loader) for val_loader in val_loaders]
    merged_model.eval()

    total_loss = []
    pbar = tqdm(range(num_steps), desc="Merging", leave=False)
    optimizer.zero_grad()
    for i in pbar:
        samples = [next(loader) for loader in cycled_val_loaders]

        # If sequnce classification, we need to handle the inputs differently
        if task in ["vqa", "captioning", "sequence_classification"]:
        ...
        if hasattr(merged_model, "loss_cluster"):
            merged_model.loss_cluster.clear()
        if task in ["vqa", "captioning"]:
            # Ada merging for VLM
            if ada:
                with autocast(dtype=torch.bfloat16):
                    outs = merged_model(
                        inputs,
                        max_new_tokens=128,
                        backprop_entropy=True,
                        report_entropies=True,
                    )
                    loss = torch.mean(
                        torch.tensor(
                            [torch.mean(torch.tensor(task_entropy)) for task_entropy in outs]
                        )
                    )
            elif kernel:
                with autocast(dtype=torch.bfloat16):
                    outs = merged_model(
                        inputs,
                        loss_cluster=merged_model.loss_cluster,
                        max_new_tokens=1,
                        use_ada=False,
                    )
                    loss += merged_model.loss_cluster.return_loss()
            else:
                raise NotImplementedError(
                    "Currently only support ada or kernel merging for VQA and Captioning tasks."
                )
        elif task in ["sequence_classification"]:
            with autocast(dtype=torch.bfloat16):
        ...
        # if loss is just a tensor without grad_fn, do not backprop
    ...
    print(f"Final Loss: {sum(total_loss) / len(total_loss):.4f}")
    return merged_model
...
def eval_indiv(
    ...
    with torch.no_grad():
        for cfg, model, test_loader, criterion, validate_fn, rep_metric, baseline in zip(
            ...
            model.to(device)
            model.eval()
            if cfg.task.name == "vqa" or cfg.task.name == "captioning":
                # Validate_fn looks different for VQA and Captioning tasks
                final_metric = validate_fn(cfg, model, test_loader.dataset)
            else:
...
def simple_merging(args, cfgs, val_loaders, test_loaders, simple_combination_type, **kwargs):
    ...
    model_dirs = args.model_dirs
    preference = torch.tensor(args.preference)  # Default preference for merging
    FINAL_RESULTS = {
    ...
    if cfgs[0].model.id.lower().startswith("llava"):
        base_model = get_base_model(cfgs[0])
        models = [base_model for _ in cfgs]
    else:
    ...
    if not retrieved_baselines:
        ...
        for cfg, model, val_loader, test_loader, criterion, validate_fn in zip(
            ...
            with torch.no_grad():
                if cfg.task.name == "vqa" or cfg.task.name == "captioning":
                    # Validate_fn looks different for VQA and Captioning tasks
                    # baselines_val.append(validate_fn(cfg, model, val_loader.dataset))
                    baselines_val.append({"TBD": 0.0})  # For VLM, baselines val is not used
                    baselines_test.append(validate_fn(cfg, model, test_loader.dataset))
                else:
    ...
    # In case of unseen tasks, re-count the model preference
    model_preference = [
...
def get_parser():
    ...
    parser.add_argument(
        "--simple_merging",
        action="store_true",
        help="Run simple merging with fixed coefficients",
    )
    parser.add_argument(
        "--num_steps",
        type=int,
        default=500,
        help="Number of optimization steps for trainable merging",
    )
    parser.add_argument(
        "--lambda_reg",
        type=float,
        default=0.0,
        help="Regularization parameter for the trainable merging model",
    )
    parser.add_argument(
        "--preference",
        type=float,
        nargs="+",
        default=[0.5, 0.5],
        help="Preference coefficients for merging, default is [0.5, 0.5]",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug mode with ipdb",
    )
    parser.add_argument(
        "--merge_type",
        type=str,
        default="weight",
        choices=["weight", "msu", "svd"],
        help="Type of merging to perform: 'weight', 'msu', or 'svd'",
    )
    parser.add_argument(
        "--lamda",
        type=float,
        default=1.0,
        help="Lambda parameter for the merging model",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
        default=1e-3,
        help="Learning rate for the optimizer in trainable merging",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="linear",
        choices=["linear", "prob", "softplus", "general", "exp", "sigmoid"],
        help=(
            "Type of coefficient scaling: "
            "'sigmoid', 'linear', 'prob', 'softplus', 'general', or 'exp'"
        ),
    )
    parser.add_argument(
        "--tracking",
        action="store_true",
        help="Enable tracking with kernel/image projector",
    )
    parser.add_argument(
        "--kernel",
        action="store_true",
        help="Enable kernel merging with projector cluster",
    )
    parser.add_argument(
        "--ada",
        action="store_true",
        help="Enable adaptive merging with the linear-combination loss",
    )
    parser.add_argument(
        "--linsearch",
        action="store_true",
        help="Enable linear search for hyperparameters",
    )
    return parser
...
if __name__ == "__main__":
    ...
    if args.linsearch and args.simple_merging:
        # linsearch_task_arithmetic(args, cfgs, val_loaders, test_loaders)
        linsearch_ties(args, cfgs, val_loaders, test_loaders)
        # linsearch_knots_ties(args, cfgs, val_loaders, test_loaders)
        # linsearch_dare_ties(args, cfgs, val_loaders, test_loaders)
